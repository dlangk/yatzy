import { CATEGORY_NAMES, CATEGORY_COUNT, TOTAL_DICE, BONUS_THRESHOLD, BONUS_SCORE, UPPER_CATEGORIES } from './constants.js';

const STORAGE_KEY = 'yatzy-game-state';

export function saveState(state) {
  try {
    const { lastEvalResponse, sortMap, ...persistable } = state;
...
function computeUpperScore(categories) {
  let sum = 0;
...
function computeTotalScore(categories, bonus) {
  let sum = 0;
...
function computeScoredMask(categories) {
  return categories.reduce(
    (mask, c) => (c.isScored ? mask | (1 << c.id) : mask), 0
  );
}

function recomputeDerived(categories) {
...
export function gameReducer(state, action) {
  switch (action.type) {
    ...
    case 'SCORE_CATEGORY': {
      ...
      const turnPhase = derived.scoredCount >= CATEGORY_COUNT ? 'game_over' : 'idle';
    ...
    case 'UNSET_CATEGORY': {
      ...
      const derived = recomputeDerived(categories);
      const turnPhase = state.turnPhase === 'game_over' ? 'idle' : state.turnPhase;
