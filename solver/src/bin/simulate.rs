...
use yatzy::simulation::{
    aggregate_statistics, make_policy, policy_thetas, save_raw_simulation, save_statistics,
    simulate_batch, simulate_batch_adaptive_with_recording, simulate_batch_with_recording,
...
fn parse_args() -> (usize, u64, Option<String>, f32, bool, Option<String>) {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "Usage: yatzy-simulate [--games N] [--seed S] [--output DIR] [--theta FLOAT] [--max-policy] [--policy NAME]"
                );
                println!();
                println!("Options:");
    ...
    (num_games, seed, output, theta, max_policy, policy)
}

fn main() {
    set_working_directory();
    let (num_games, seed, output, theta, max_policy, policy_name) = parse_args();
