# Optimal Scandinavian Yatzy: Algorithm and Optimizations
...
### 5.2 f32 storage + reachability pruning (1.6×)
...
### 5.4 Keep-EV dedup (9× fewer dot products)
...
### 5.5 State index layout swap (45× faster Group 6)
...
### 5.6 Grouped parallelism (amplifies 5.5)
...
### 5.7 Lower-category successor preloading
...
## 6. Target Hardware: Apple M1 Max
...
### 6.3 AMX (Apple Matrix Extensions)
...
**Relevance to solver**: The KeepTable CSR dot product is a SpMV operation. If reformulated as a dense MatVec (462 × 252 × f32), it could potentially be dispatched via `cblas_sgemv` → AMX. This would bypass the NEON gather limitation entirely. See §7.1.

### 6.4 Firestorm P-core Microarchitecture
...
### 6.6 Memory Hierarchy for Solver Data Structures
...
**Total L1 working set per widget**: ~105 KB — fits within the 128 KB L1D$. This is why Steps 5.3 (keep-multiset dedup) and 5.4 (keep-EV dedup) were so effective: they collapsed the working set from L2 to L1.
...
### 7.3 Bottleneck analysis: the gather problem
...
### 8.1 Dense MatVec via AMX / Accelerate
...
### 8.2 Manual loop unrolling with multiple accumulators
...
### 8.3 Batched widget dispatch to GPU (Metal)
...
**Challenges**:
1. **Step 1 (sparse dot product)**: Same gather problem as CPU — Metal has no native scatter-gather. Would need to use the dense MatVec approach (§8.1) on GPU, where it would be trivially parallelized across the 32 GPU cores.
...
### 8.4 SIMD-parallel Group 6 (SoA restructure)

**Idea**: Restructure Group 6 to process 4 dice sets simultaneously in one NEON register:

```
// Process ds_i = {0,1,2,3} simultaneously
V_scr = vld1q_s32(&precomputed_scores[ds_i..ds_i+4][c])   // 4 scores
V_succ = vld1q_f32(&sv[successor_indices])                  // 4 successor EVs (gather!)
V_val = vadd(vcvt(V_scr), V_succ)                          // 4 candidate values
V_best = vmax(V_best, V_val)                                // 4 running maxima
```
...
### 8.5 Precomputed unscored-category tables

**Idea**: For each of the $2^{15} = 32{,}768$ scored bitmasks, precompute a compact list of unscored category indices. Replace the inner loop:
...
```rust
let unscored = &UNSCORED_TABLE[scored];
for &c in unscored { ... }
```

**Cost**: 32,768 entries × ~7.5 categories avg × 1 byte = ~246 KB. Exceeds L1 but fits in L2. However, at any given level only $\binom{15}{|C|}$ masks are active (e.g., 3,003 at $|C|=5$), so the hot set is much smaller.

**Benefit**: Eliminates the branch `is_category_scored` (bit test + conditional branch). On Firestorm with its aggressive branch predictor and 8-wide decode, this branch is very cheap (~0.5 cycles amortized). The table lookup costs a load instead. Net effect is probably slightly negative or neutral.

**Verdict**: Not worth it. The branch predictor handles the `is_category_scored` pattern well, and the table adds memory pressure.

### 8.6 Multi-level fusion (Groups 5+3)

**Idea**: Fuse Groups 5 and 3 into a single pass. Currently:

```
Step 1a: keep_ev_5[462] = CSR × e[0]      // Group 5 Step 1
Step 2a: e[1] = max over keep_ev_5         // Group 5 Step 2
Step 1b: keep_ev_3[462] = CSR × e[1]      // Group 3 Step 1
Step 2b: e[0] = max over keep_ev_3         // Group 3 Step 2
```
...
**Alternative**: Could we compute `keep_ev_3[kid] = max over dice sets of keep_ev_5[kid']` for some transformation? No — Group 3's input is the *max over keeps* from Group 5 for each dice set, which depends on the dice-set-specific keep structure. The max-over-keeps operation in Step 2a is not distributive over the CSR multiply in Step 1b.

**Verdict**: Not fusible without fundamentally changing the algorithm.

### 8.7 f16 (half-precision) computation

**Idea**: ARMv8.2-A includes fp16 arithmetic. Switching from f32 to f16 would double NEON throughput (8 lanes instead of 4) and halve memory bandwidth requirements.

**Analysis**:
- f16 has ~3.3 significant decimal digits (10-bit mantissa). Game EVs range 0–400; an f16 ULP at 256 is 0.25 points.
- The upper bonus cliff at m=63 creates a sharp discontinuity where 0.25-point precision could flip decisions.
- The 462 keep EVs accumulate ~9.5 FMAs on average. Rounding error accumulates as $O(\sqrt{n}) \cdot \text{ULP}$ ≈ $3 \times 0.25 = 0.75$ points — unacceptable.

**Verdict**: f16 precision is insufficient for the DP accumulation. The f32→f16 validation would almost certainly show decision-impacting flips near the bonus cliff. Not viable.

### 8.8 Precomputed Group 6 results for common scored masks

**Idea**: The terminal levels (|C| = 13, 14) have very few unscored categories (2 or 1). For |C| = 14 (1 unscored category), Group 6 is trivial: `e[0][ds_i] = score + sv[successor]`. For |C| = 13 (2 unscored), it's a max of 2 values. These levels have many widgets (6,435 at |C| = 13; 960 at |C| = 14) but each widget's Group 6 is very cheap.

**Analysis**: At these levels, Groups 5+3 dominate even more (the 81% ratio increases because Group 6 shrinks). Optimizing Group 6 at high |C| has negligible impact on wall clock. The important levels are |C| = 0–8, where Group 6 iterates 7–15 categories and the sv lookups are the hot path.

**Verdict**: Correct observation but targeting the wrong bottleneck. Groups 5+3 (81%) is where the time is.
