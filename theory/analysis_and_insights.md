...
Two plots generated by `yatzy-analyze plot` (source: `analytics/src/yatzy_analysis/plots/percentiles.py`):

- **`outputs/plots/percentiles_vs_theta.png`** — Score percentiles (p5 through p99) plus min/max vs θ across the full 37-point grid [-3, +3]. Shows the fan-shaped divergence of percentile curves as |θ| increases, with lower percentiles peaking at negative θ and upper percentiles peaking at positive θ.
- **`outputs/plots/percentiles_vs_theta_zoomed.png`** — Same curves zoomed to θ ∈ [-1, +1], the active regime where policy changes are meaningful. The crossover where risk-seeking starts to pay off for upper percentiles (θ ≈ 0.03-0.04) and where risk-averse peaks for lower percentiles (θ ≈ -0.03 to -0.05) are clearly visible.

Additional score distribution plots generated by `yatzy-analyze plot` (source: `analytics/src/yatzy_analysis/plots/`):

- **`outputs/plots/cdf_full.png`** — Full CDF curves for all θ values, colored by θ. Shows how risk-seeking shifts mass rightward in the tail.
- **`outputs/plots/tails_zoomed.png`** — Zoomed survival curves (1-CDF) for the upper and lower tails.
- **`outputs/plots/density.png`** — KDE density estimates for all θ values, showing distribution shape changes.
- **`outputs/plots/quantile.png`** — Quantile-quantile comparisons across θ values.
- **`outputs/plots/combined.png`** — Multi-panel composite: CDF, density, percentiles, and mean-std in one figure.
- **`outputs/plots/efficiency.png`** — 4-panel efficiency metrics: MER (marginal exchange rates), SDVA (stochastic dominance violation area), CVaR deficit, and combined frontier. Generated by `yatzy-analyze efficiency`.
...
- **`outputs/plots/mean_vs_std.png`** — Mean vs standard deviation scatter colored by θ, showing the two-branch structure. Generated by `plot_mean_vs_std()` in `analytics/src/yatzy_analysis/plots/mean_std.py`.
...
Generated by `yatzy-analyze categories` (source: `analytics/src/yatzy_analysis/plots/categories.py`). Full and zoomed (|θ| ≤ 1) variants:

- **`outputs/plots/category_score_heatmaps.png`** — Heatmap of mean score per category across θ values.
- **`outputs/plots/category_rate_heatmaps.png`** — Heatmap of zero-rate (dump rate) per category across θ.
- **`outputs/plots/category_fill_turn_heatmaps.png`** — Heatmap of mean fill-turn per category across θ.
- **`outputs/plots/category_sparklines_mean_score.png`** — Per-category sparkline of mean score vs θ.
- **`outputs/plots/category_sparklines_zero_rate.png`** — Per-category sparkline of zero rate vs θ.
- **`outputs/plots/category_sparklines_mean_fill_turn.png`** — Per-category sparkline of fill turn vs θ.
- **`outputs/plots/category_bars.png`** — Grouped bar chart comparing selected θ values.
- **`outputs/plots/category_slope.png`** — Slope chart showing category rank changes between θ=0 and θ=3.
- **`outputs/plots/category_slope_dense.png`** — Dense slope chart with multiple θ values.
- **`outputs/plots/category_fill_turn_heatmap.png`** — Fill-turn heatmap (alternative layout).
...
Generated by `yatzy-analyze yatzy-hypothesis` (source: `analytics/src/yatzy_analysis/plots/yatzy_hypothesis.py`):

- **`outputs/plots/yatzy_conditional_bars.png`** — Conditional Yatzy hit rate by score band and θ.
- **`outputs/plots/yatzy_unconditional_vs_tail.png`** — Unconditional vs top-5% Yatzy hit rate across θ.
- **`outputs/plots/yatzy_dump_gap.png`** — Mean score gap between Yatzy-hit and Yatzy-miss games.
...
Generated by `yatzy-analyze state-flow` (source: `analytics/src/yatzy_analysis/plots/state_flow.py`):

- **`outputs/plots/state_flow_bonus_alluvial.png`** — Alluvial diagram showing upper-section bonus progression across turns.
- **`outputs/plots/state_flow_dag.png`** — Directed acyclic graph of state transitions.
- **`outputs/plots/state_flow_streamgraph.png`** — Streamgraph of category fill patterns across turns.

Generated by `yatzy-analyze sensitivity` (source: `analytics/src/yatzy_analysis/plots/sensitivity.py`):

- **`outputs/plots/sensitivity_flip_rates.png`** — Decision flip rates by type and game phase.
- **`outputs/plots/sensitivity_flip_theta.png`** — Distribution of θ values where flips occur.
- **`outputs/plots/sensitivity_gaps.png`** — EV gap distributions at flip points.
- **`outputs/plots/sensitivity_heatmap.png`** — Turn × decision type flip heatmap.

Scenario cards from `generate_example_cards()` in `analytics/src/yatzy_analysis/plots/scenario_cards.py`:

- **`outputs/plots/scenario_card_{hex_id}.png`** — Individual scenario cards showing scorecard, advantage chart, and θ table for the most interesting decision flips.
...
Three plots generated by `generate_frontier_plots()` (source: `analytics/src/yatzy_analysis/plots/frontier.py`):

- **`outputs/plots/frontier_pareto.png`** — Mean vs σ scatter showing the constant-θ baseline curve (blue) and adaptive policy points (colored diamonds). Vertical dashed lines connect each adaptive point to the frontier at matched σ, visualizing the Δμ gap.
- **`outputs/plots/frontier_cdf.png`** — Overlaid CDFs for all policies. Thin blue lines for constant-θ baselines, thick colored lines for adaptive policies. Shows that adaptive policies produce nearly identical score distributions to nearby constant-θ baselines.
- **`outputs/plots/frontier_delta.png`** — Horizontal bar chart of Δμ for each adaptive policy, with H1 threshold at +1.0. All bars are negative, visually confirming that no policy beats the frontier.
