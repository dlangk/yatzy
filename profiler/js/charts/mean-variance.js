...
export async function initMeanVariance() {
  const data = await DataLoader.sweepSummary();
  const container = document.getElementById('chart-mean-variance');
  if (!container) return;
  ...
  const chart = createChart('chart-mean-variance-svg', { aspectRatio: 0.6 });
  if (!chart) return;
