...
void RespondWithJSON(struct MHD_Connection *connection, int status_code, json_object *json_resp) {
...
void RespondWithError(struct MHD_Connection *connection, int status_code, const char *error_msg) {
...
int ParseJSONField(json_object *json_obj, const char *field, int *out_val) {
...
int ParseDiceArray(json_object *json_obj, int dice[5]) {
...
int ValidateAndParseJSON(json_object *parsed, const char *fields[], int num_fields, int *out_values[]) {
...
int ParseDiceFromJSON(json_object *parsed, int dice[5]) {
...
void AddJSONField(json_object *json_resp, const char *key, int value) {
    json_object_object_add(json_resp, key, json_object_new_int(value));
}

void AddJSONFieldDouble(json_object *json_resp, const char *key, double value) {
...
void handle_evaluate_category_score(YatzyContext *ctx, struct MHD_Connection *connection, json_object *parsed) {
    int dice[5];
...
void handle_available_categories(YatzyContext *ctx, struct MHD_Connection *connection, json_object *parsed) {
    ...
    for (int c = 0; c < CATEGORY_COUNT; c++) {
        ...
        bool valid = !IS_CATEGORY_SCORED(scored_categories, c) && scr > 0;
...
void handle_evaluate_all_categories(YatzyContext *ctx, struct MHD_Connection *connection, json_object *parsed) {
    int upper_score, scored_categories, rerolls, dice[5];
    const char *fields[] = {"upper_score", "scored_categories", "rerolls_remaining"};
    int *values[] = {&upper_score, &scored_categories, &rerolls};
...
void handle_evaluate_actions(YatzyContext *ctx, struct MHD_Connection *connection, json_object *parsed) {
    int upper_score, scored_categories, rerolls, dice[5];
    const char *fields[] = {"upper_score", "scored_categories", "rerolls_remaining"};
    int *values[] = {&upper_score, &scored_categories, &rerolls};
...
void handle_suggest_optimal_action(YatzyContext *ctx, struct MHD_Connection *connection, json_object *parsed) {
    int upper_score, scored_categories, rerolls, dice[5];
    const char *fields[] = {"upper_score", "scored_categories", "rerolls_remaining"};
    int *values[] = {&upper_score, &scored_categories, &rerolls};
...
void handle_evaluate_user_action(YatzyContext *ctx, struct MHD_Connection *connection, json_object *parsed) {
    int upper_score, scored_categories, rerolls, dice[5];
    const char *fields[] = {"upper_score", "scored_categories", "rerolls_remaining"};
    int *values[] = {&upper_score, &scored_categories, &rerolls};
...
void handle_get_score_histogram(YatzyContext *ctx, struct MHD_Connection *connection) {
    ...
    fgets(line, MAX_LINE_LENGTH, file); // Skip header if present
...
typedef void (*POSTHandler)(YatzyContext *, struct MHD_Connection *, json_object *);
...
void LogRequest(const char *method, const char *url, const char *payload, int status_code) {
    time_t now = time(NULL);
    char time_str[26];
    ctime_r(&now, time_str);
    time_str[strcspn(time_str, "\n")] = '\0';
    printf("[%s] [%s] %s - Payload: %s - Status: %d\n",
           time_str, method, url, payload ? payload : "N/A", status_code);
}
